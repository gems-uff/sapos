...
class ChangeAllocationTimeColumnsToInteger < ActiveRecord::Migration[5.1]
  def self.up
    Allocation.transaction do
      ...
      allocations.each do |allocation|
        ...
        allocation_for_update.update_attributes!(:start_time => allocation[:start_time], :end_time => allocation[:end_time])
      end
    end
  end
  ...
  def self.down
    Allocation.transaction do
      allocations = Allocation.all.map{|allocation| {:id => allocation.id, :start_time => allocation.start_time, :end_time => allocation.end_time} }
      ...
      allocations.each do |allocation|
        allocation_for_update = Allocation.where(:id => allocation[:id]).last
        allocation_for_update.update_attributes!(:start_time => (Time.zone.parse(standard_date.strftime('%Y/%m/%d')) + allocation[:start_time].hours).to_datetime, :end_time => (Time.zone.parse(standard_date.strftime('%Y/%m/%d')) + allocation[:end_time].hours))
      end
    end
  end
end
