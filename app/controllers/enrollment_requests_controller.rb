...
class EnrollmentRequestsController < ApplicationController
  ...
  active_scaffold :"enrollment_request" do |config|
    ...
    config.list.columns = [
      :year, :semester, :enrollment, :status,
      :last_student_change_at, :last_staff_change_at
    ]
    config.update.columns = [
      :class_enrollment_requests, :enrollment_request_comments
    ]
    config.show.columns = [
      :year, :semester, :enrollment, :status, :last_student_change_at,
      :last_staff_change_at, :class_enrollment_requests,
      :enrollment_request_comments
    ]
    ...
    config.update.link.label = "
      <i title='#{I18n.t("enrollment_request.validate.link")}'
  ...
  record_select(
    per_page: 10,
    search_on: [:year, :semester, :enrollment],
    order_by: "year DESC, semester DESC",
  ...
  def self.condition_for_status_column(column, value, like_pattern)
    custom_condition_for_status(
      value, "enrollment_requests.id in", "enrollment_requests.id not in", ""
    )
  end
    ...
    def before_update_save(record)
      ...
      record.class_enrollment_requests.each do |cer|
        ...
        if cer.status_changed?
          result[:change] << cer
          is_effected = cer.status == ClassEnrollmentRequest::EFFECTED
          if is_effected && cannot?(:effect, cer)
      ...
      if message.present?
        @comment = EnrollmentRequestComment.new(
