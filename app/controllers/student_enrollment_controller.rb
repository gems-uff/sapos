...
class StudentEnrollmentController < ApplicationController
  ...
  def show
    ...
    if @enrollment.dismissal.blank?
      existing_partial(
        "student_enrollment/show_enroll",
        :semester,
        ClassSchedule.find_by(ClassSchedule.arel_enroll_open?)
      ) do |semester|
    ...
    existing_partial(
      "student_enrollment/show_dismissal", :dismissal, @enrollment.dismissal,
      enrollment: @enrollment,
      thesis_defense_committee_professors:
        @enrollment.thesis_defense_committee_professors
    )
    existing_partial(
      "student_enrollment/show_class_enrollments",
      :class_enrollments, @enrollment.class_enrollments
    )
    existing_partial(
      "student_enrollment/show_advisements",
      :advisements, @enrollment.advisements
    )
    existing_partial(
      "student_enrollment/show_phases",
      :phase_completions, @enrollment.completed_or_active_phase_completions
    )
    existing_partial(
      "student_enrollment/show_deferrals", :deferrals, @enrollment.deferrals
    )
    existing_partial(
      "student_enrollment/show_holds", :holds, @enrollment.enrollment_holds
    )
    existing_partial(
  ...
  private
    def _valid_enrollment
      ...
      raise CanCan::AccessDenied.new if @enrollment.blank?
      raise CanCan::AccessDenied.new if @enrollment.student.user != current_user
      raise CanCan::AccessDenied.new if !@enrollment.enrollment_status.user
      nil
    end
    ...
    def _redirect_semester(check_time)
      ...
      if @enrollment.dismissal.present?
        return redirect_to student_enrollment_path(@enrollment.id),
          alert: I18n.t(
            "student_enrollment.alert.dismissed_enrollment",
      ...
      if check_time && ! @semester.enroll_open?
        return redirect_to student_enrollment_path(@enrollment.id),
          alert: I18n.t(
            "student_enrollment.alert.closed_enrollment",
            year: params[:year],
    ...
    def _prepare_enroll(check_time)
      ...
      @advisement_authorizations = Professor
        .joins(:advisement_authorizations).order(:name).distinct
      @enrollment_request = EnrollmentRequest.find_or_initialize_by(
        enrollment: @enrollment,
        year: @semester.year,
        semester: @semester.semester
    ...
    def enrollment_request_params
      params.require(:enrollment_request).permit(
        :delete_request, :message, course_class_ids: [],
        course_ids: [:selected, :professor]
      )
    end
    ...
    def existing_partial(partial, attribute, association, others = {})
      ...
      @partials << [
        partial,
        { attribute => association }.merge(others).merge(extra)
      ]
    end
    ...
    def save_enrollment_request_and_redirect
      ...
      if changed
        @comment = @enrollment_request.enrollment_request_comments.build(
          message: message, user: current_user
        ) unless message.empty?
        @enrollment_request.student_change!
      end
      if @enrollment_request.save_request
        notify_enrollment_request_change(
          @enrollment_request, request_change, message, current_user
        ) if changed
        return redirect_to student_enrollment_path(@enrollment.id),
          notice: I18n.t("student_enrollment.notice.request_saved")
      end
    ...
    def notify_enrollment_request_change(
      enrollment_request, request_change, message, user
    )
      attributes = {
