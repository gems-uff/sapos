...
module EnrollmentSearchConcern extend ActiveSupport::Concern
  ...
  class_methods do
    ...
    def custom_condition_for_scholarship_durations_active_column(column, value, like_pattern)
    ...
    def custom_condition_for_course_class_year_semester_column(column, value, like_pattern)
      return [] if value[:year].empty? && value[:semester].empty? && value[:course].empty?
  
      result = []
  
      search_sql = " enrollments.id in ( select class_enrollments.enrollment_id 
