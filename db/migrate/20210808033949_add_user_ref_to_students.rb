class AddUserRefToStudents < ActiveRecord::Migration[6.0]
  def up
    ...
    change_column :professors, :user_id, :integer, limit: 8, unique: true, null: false, auto_increment: true
    add_index :professors, :user_id, name: "index_professors_on_user_id" unless index_exists?(:professors, :user_id)
    change_column :users, :invited_by_id, :integer, limit: 8, unique: true, null: false, auto_increment: true
