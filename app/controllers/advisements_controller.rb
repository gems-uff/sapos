...
class AdvisementsController < ApplicationController
  ...
  active_scaffold :advisement do |config|
    ...
    config.columns.add :active, :co_advisor, :enrollment_number
    config.columns.add :student_name, :level

    config.field_search.columns = [
      :professor, :enrollment_number, :level, :student_name,
      :main_advisor, :active, :co_advisor
    ]
    config.list.columns = [
      :professor, :enrollment, :main_advisor, :active, :co_advisor
    ]
    config.create.columns = [:professor, :enrollment, :main_advisor]
    config.update.columns = [:professor, :enrollment, :main_advisor]
    config.show.columns = [
      :professor, :enrollment, :main_advisor, :co_advisor_list
    ]
    ...
    config.columns[:enrollment_number].sort_by(
      sql: "enrollments.enrollment_number"
    )
    ...
    respond_to do |format|
      format.pdf do
        send_data render_to_string,
          filename: "#{I18n.t("pdf_content.advisements.to_pdf.filename")}.pdf",
