...
module StudentEnrollmentHelper
  def find_class_enrollment_request_for_class_schedule_table_row(enrollment_request, row)
    cer = nil 
  ...
  def display_class_schedule_table_row?(semester, cer)
    semester.main_enroll_open? || semester.adjust_enroll_insert_open? || cer.present?
  end
  ...
  def on_demand_row_style(cer)
    return '' if cer.blank?
    return '' if cer.action == ClassEnrollmentRequest::REMOVE && cer.status == ClassEnrollmentRequest::EFFECTED
  ...
  def cer_row_status(cer)
    ...
    row_status = "#{cer.db_action} #{row_status}" if cer.db_action == ClassEnrollmentRequest::REMOVE
  ...
  def enroll_table_td(options)
    ...
    content_tag(:td, class: tdclass) do
      options[:form].label options[:labelid], for: "table_row_#{options[:row_index]}", class: labelclass  do
