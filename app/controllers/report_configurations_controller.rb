...
class ReportConfigurationsController < ApplicationController
  ...
  active_scaffold :report_configuration do |config|
    ...
    columns = [
      ...
      :preview, :use_at_report, :use_at_transcript, :use_at_assertion,
    ...
    config.list.columns = [
      :name, :order, :text, :signature_type, :use_at_assertion,
  ...
  private
    def record_params
      params.required(:record).permit(
        ...
        :use_at_schedule, :text, :image, :order, :scale, :use_at_assertion,
