  ...
  describe "Methods" do
    ...
    describe "notify_professor" do
      it "should send email to professor when class enrollment is created" do
        ...
        @destroy_later << course_class = FactoryBot.create(:course_class, professors: [professor], course: course, year: 2026, semester: 2)
        ...
        expect(deliveries.last.to).to eq([course_class.professors_emails])
      ...
      it "should not send email to professor when it is not on adjust period" do
        ...
        if deliveries.present?
          expect(deliveries.last.to).not_to eq([course_class.professors_emails])
