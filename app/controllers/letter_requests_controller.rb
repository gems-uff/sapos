...
class LetterRequestsController < ApplicationController
  ...
  def fill
    ...
    if @letter_request.is_filled?
      ...
      @letter_fields = FormField.where(
        form_template_id: @letter_request.student_application
          .application_process.letter_template_id
      ).where.not(field_type: ["text", "file"])
      @letter_texts = FormField.where(
        form_template_id: @letter_request.student_application
        .application_process.letter_template_id, field_type: "text"
      )
      @letter_files = FormField.where(
        form_template_id: @letter_request.student_application
        .application_process.letter_template_id, field_type: "file"
      )
  ...
  private
    ...
    def letter_request_params
      params.require(:letter_request).permit(
        :email, :student_application_id, :is_filled,
        letter_field_inputs_attributes: [:form_field_id, :input, :_destroy],
        letter_text_inputs_attributes: [
          :id, :letter_request_id, :form_field_id, :input, :_destroy
        ],
        letter_file_uploads_attributes: [
          :id, :letter_request_id, :form_field_id, :file, :_destroy
        ],
      )
    end
end
