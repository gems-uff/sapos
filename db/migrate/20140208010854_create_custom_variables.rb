...
class CreateCustomVariables < ActiveRecord::Migration
  def up
    ...
    CustomVariable.where(:variable => "notification_frequency").first or CustomVariable.create(:name=>"Frequência de envio de notificações", :variable =>"notification_frequency", :value => "1d")
    CustomVariable.where(:variable => "notification_start_at").first or CustomVariable.create(:name=>"Horário inicial para o envio de notificações", :variable =>"notification_start_at", :value => "4:00")
    CustomVariable.where(:variable => "class_schedule_text").first or CustomVariable.create(:name=>"Texto no final do quadro de horários", :variable =>"class_schedule_text", :value => "Alunos interessados em cursar disciplinas de Tópicos Avançados devem consultar os respectivos professores antes da matrícula")
