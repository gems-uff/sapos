...
class ScholarshipsController < ApplicationController
  ...
  active_scaffold :scholarship do |config|
    ...
    config.create.columns = [
      :scholarship_number, :level, :sponsor, :scholarship_type,
      :professor, :start_date, :end_date, :obs
    ]
    config.update.columns = [
      :scholarship_number, :level, :sponsor, :scholarship_type,
      :professor, :start_date, :end_date, :obs, :scholarship_durations
    ]
    config.show.columns = [
      :scholarship_number, :level, :sponsor, :scholarship_type,
  ...
  record_select(
    per_page: 10, search_on: [:scholarship_number],
    order_by: "scholarship_number", full_text_search: true
  )
    ...
    if value[:use] == "yes"
      ...
      allocated = ScholarshipDuration
        .where(
          ...
          .and(
            (scholarship_durations[:end_date].gt(end_of_previous_month)
      ...
      if allocated.empty?
        condition1 = scholarships[:id].eq(scholarships[:id])
      else
        condition1 = scholarships[:id].not_in(allocated)
      end
      [condition1
        .and(scholarships[:start_date].lteq(dt))
        .and(
          scholarships[:end_date].eq(nil)
